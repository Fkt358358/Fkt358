...
<body class="gray-bg">
			...
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveGoods.htm">
						<input type="hidden" name="typeid" id="hidden_txt_userid" value=""/>
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								<td>所属类型：</td>
								<td colspan="4">
									<select class="form-control" name="id" id = "id" aria-required="true" required>
										<option value="">---请选择---</option>
										<c:forEach items="${tlist }" var="r">
										 	<option value="${r.id }">${r.typename }</option>
										</c:forEach>
	...
	function initEditUser(getSelection){
		$('#hidden_txt_userid').val(getSelection.typeid);
